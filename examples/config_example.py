...
def demo_basic_config():
    ...
    print("   💡 You can edit this file and use: ck.configure(config_file='example_config.json')")
...
def demo_advanced_features():
    ...
    try:
        ...
        print(f"✅ Loaded config with {len(config.get_provider_config('google'))} Google models")
        ...
        for model_name, prompt in models_to_test:
            try:
                ...
                response = model.generate_content(prompt[:50] + "...")  # Truncate for demo
                ...
                if response.usage_metadata.optimizations_applied:
                    print(f"   ⚡ Optimizations: {', '.join(response.usage_metadata.optimizations_applied)}")
...
def demo_cost_controls():
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Optimized request successful (cost: ${response.usage_metadata.cost:.4f})")
